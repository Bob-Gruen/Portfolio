class Simulation {
    constructor(canvas) {
        ...
        // Config
        this.NUM_CREATURES = 150;
        this.NUM_FOOD_ITEMS = 750;
        this.FOOD_SPAWN_INTERVAL = 50;
        this.GENERATION_TIMER = 30; // Increased generation time
        this.NETWORK_CONFIG = {
            inputSize: 30, // 5 rays * 5 inputs + 5 self-awareness = 30
            hiddenLayers: [64, 32],
            outputSize: 4, // Relative Controls (Thrust, Left, Right, Wait)
    ...
    _updateNetworkVisualization() {
        ...
        // Fetch nearby objects for visualization context
        const nearbyObjects = this.spatialGrid.getNearby(creatureToVisualize.body.position.x, creatureToVisualize.body.position.y, this.SIGHT_DISTANCE);
        const inputs = creatureToVisualize.getNetworkInputs(nearbyObjects);
    ...
    _drawNetworkSimple(ctx, bestCreature, net, inputs, activations) {
        ...
        for (let i = 0; i < layerPositions.length - 1; i++) {
            for (let j = 0; j < layerPositions[i].length; j++) {
                ...
                // i=0 is Input->H1, uses inputs.
                // i=1 is H1->H2, source is H1 (outputs[0]). So we use i-1.
                const sourceActivation = (i === 0) ? inputs[j] : (bestCreature.net.outputs[i - 1] ? bestCreature.net.outputs[i - 1][j] : 0);

                for (let k = 0; k < (layerPositions[i + 1] || []).length; k++) {
                    // Safety check: ensure weights exist for this connection
                    // Use i+1 because weights for connection A->B are stored in layer B
                    if (!netJSON.layers[i + 1].weights || !netJSON.layers[i + 1].weights[k]) continue;
    ...
    calculateFitness(creature) {
        ...
        if (creature.totalActions > 0 && (creature.actionsTaken[3] / creature.totalActions) > 0.9) {
    ...
    evolvePopulation() {
        ...
        if (this.creatures.length > 1) {
            ...
            const eliteCount = Math.min(Math.floor(this.creatures.length * 0.40), this.NUM_CREATURES - 1); // Cap elites to new population size
            ...
            while (newCreatures.length < this.NUM_CREATURES) {
    ...
    resetWorld(fullReset = false) {
        this.isRunning = false;
        this.generationCount++;

        if (fullReset) {
            // Fix: Clean up existing creature bodies before wiping references to prevent physics engine leaks
            for (let creature of this.creatures) {
                this.World.remove(this.engine.world, creature.body);
                this.World.remove(this.engine.world, creature.leftArm);
                this.World.remove(this.engine.world, creature.rightArm);
            ...
            this.creatures = []; // Wipe population to force fresh start with new config
            this.bestCreatureEver = null;
            this.bestFitnessEver = -Infinity;
            this.generationCount = 1; // Reset generation count on full reset
        }
    ...
    afterRender() {
        ...
        if (this.drawRays) {
            for (const creature of this.creatures) {
                if (creature.energy > 1 && creature.rayResults && creature.rayResults.length > 0) {
                    const startPos = creature.body.position;
                    // Re-calculate ray angles to match what castRays does.
                    // Ideally we should cache end points but re-calc is cheap enough for viz.
                    const baseAngle = creature.body.angle;
                    const fov = creature.RAY_FOV;
                    const rayCount = creature.RAY_COUNT;
                    const angleStep = fov / (rayCount - 1);
                    const startAngle = baseAngle - (fov / 2);

                    creature.rayResults.forEach((result, index) => {
                        const angle = startAngle + (index * angleStep);
                        const dist = result.distance * this.SIGHT_DISTANCE;
                        const endX = startPos.x + Math.cos(angle) * dist;
                        const endY = startPos.y + Math.sin(angle) * dist;

                        this.ctx.beginPath();
                        this.ctx.moveTo(startPos.x, startPos.y);
                        this.ctx.lineTo(endX, endY);

                        let color = 'rgba(255, 255, 255, 0.2)'; // Default: Nothing seen (max dist)
                        if (result.distance < 1) {
                            switch (result.type) {
                                case 'food': color = 'lime'; break;
                                case 'poison': color = 'red'; break;
    ...
    _simulationLoop() {
        ...
        if (this.isRunning) {
            for (let t = 0; t < this.creatures.length; t++) {
                ...
                curr_creature.updateMetabolism(this.deltaTime);
                ...
                // Input Gathering: Pass all nearby objects to the creature's raycasting system
                let inputs = [];
                if (curr_creature.energy > 0) {
                    // Optimized: Get nearby objects from spatial grid once
                    ...
                    // Filter out duplicate IDs if necessary, but spatial grid usually handles this well enough for collision candidates.
                    // Important: The creature needs the MATTER BODIES to raycast against.

                    // Update the creature's "nearestObjects" for visualization (optional, or remove if unused)
                    // We'll trust getNetworkInputs to populate rayResults for viz.

                    inputs = curr_creature.getNetworkInputs(nearbyObjects);
                }

                if (curr_creature.energy > 0) {
                    const maxForce = 0.01;
                    const move_energy_loss = 0.005;
                    let forceX = 0;
                    let forceY = 0;

                    // New Relative Control Scheme (Output Size: 4)
                    const action = curr_creature.getNetworkOutput(inputs);

                    // Track action for passivity penalty
                    curr_creature.totalActions++;
                    curr_creature.actionsTaken[action]++;

                    switch (action) {
                        case 0: // Thrust Forward
                            const angle = curr_creature.body.angle;
                            forceX = Math.cos(angle) * maxForce * 2.5; // Strong forward thrust
                            forceY = Math.sin(angle) * maxForce * 2.5;
                            curr_creature.energy -= move_energy_loss;
                            break;
                        case 1: // Rotate Left
                            Matter.Body.setAngularVelocity(curr_creature.body, -0.15); // Faster rotation
                            curr_creature.energy -= move_energy_loss * 0.5;
                            break;
                        case 2: // Rotate Right
                            Matter.Body.setAngularVelocity(curr_creature.body, 0.15);
                            curr_creature.energy -= move_energy_loss * 0.5;
                            break;
                        case 3: // Wait / Interact
                            // Recover a tiny bit of energy? Or just standard metabolic rate?
    ...
    updateTimers() {
        ...
        if (this.lastUpdateTime > 0) {
            ...
            this.deltaTime = elapsedMilliseconds;
